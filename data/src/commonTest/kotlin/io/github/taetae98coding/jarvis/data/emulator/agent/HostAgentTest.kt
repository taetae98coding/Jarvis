...
class HostAgentTest {
    ...
    fun missingAgentMeansUncountable() = runTest {
        val statuses = hostAgentEmulatorDataSource { null }.observeStatus().take(1).toList()
    ...
    fun dataSourceFollowsAgentUpdates() = runTest {
        ...
        backgroundScope.launch { hostAgentEmulatorDataSource { answer }.observeStatus().toList(statuses) }
        runCurrent()
