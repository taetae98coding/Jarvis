...
class JarvisAppTerminalCodeNavigationTest {
    ...
    // 작업 트리 파일 탭은 열자마자 편집 보기다(docs/common/terminal-file-editor.html E2).
    ...
    fun commandBJumpsToTheOnlyDeclarationInAnotherFile() = runComposeUiTest {
        ...
        // 옮겨 간 탭도 편집 보기라 커서가 선언 자리(3번째 줄 5번째 칸)에 온다.
        waitUntil(timeoutMillis = FrameTimeoutMillis) {
            runCatching { onNodeWithTag(TerminalFileViewerEditorTestTag).fetchSemanticsNode().config[SemanticsProperties.TextSelectionRange] }.getOrNull() == TextRange(otherText.indexOf("greet"))
        }
    }
