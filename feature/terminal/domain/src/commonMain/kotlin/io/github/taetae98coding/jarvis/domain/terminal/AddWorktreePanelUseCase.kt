...
class AddWorktreePanelUseCase(
    ...
    suspend operator fun invoke(parentId: Long, branch: String, path: String): Result<TerminalWorkspaceChange> {
        ...
        return gitRepository.addWorktree(repository, name, directory).map { worktree ->
            updateWorkspace { it.addWorktreePanel(parentId, name, worktree.path) }
        }
    }
