...
/**
 * [parentId] 패널의 폴더가 속한 저장소에 워크트리를 만들고, 성공하면 그 폴더를 가진 빈 패널을 부모 아래에 붙인다.
...
class AddWorktreePanelUseCase(
    ...
    suspend operator fun invoke(parentId: Long, branch: String, baseBranch: String?, path: String): Result<TerminalWorkspaceChange> {
        ...
        return gitRepository.addWorktree(repository, name, directory, base).map { worktree ->
            updateWorkspace { it.addWorktreePanel(parentId, name, worktree.path, branch = name, baseBranch = base) }
        }
    }
