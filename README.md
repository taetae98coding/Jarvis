...
공용 코드는 **기능마다 clean architecture 세 계층을 모듈로** 갖는다. 기능은 `appinfo` · `emulator` · `screen` · `rotation` · `terminal` · `theme` · `profiling` · `battery` · `focus` · `devtools` · `unitconverter` · `texttools` · `calculator` · `mcp` 열넷이고 서로를 의존하지 않는다.
...
androidApp   iosApp(Xcode)   desktopApp   webApp
     ...
     ┌───────────────┼───────────────┬───────────────┐
 feature:appinfo  feature:emulator  feature:screen  feature:terminal  …  (기능 열넷)
