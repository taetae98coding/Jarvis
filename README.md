...
공용 코드는 **기능마다 clean architecture 세 계층을 모듈로** 갖는다. 기능은 `appinfo` · `emulator` · `screen` · `rotation` · `terminal` · `theme` · `profiling` · `battery` · `focus` · `devtools` · `unitconverter` · `texttools` · `calculator` · `qrcode` · `worldclock` · `mcp` 열여섯이고 서로를 의존하지 않는다.
...
androidApp   iosApp(Xcode)   desktopApp   webApp
     ...
     ┌───────────────┼───────────────┬───────────────┐
 feature:appinfo  feature:emulator  feature:screen  feature:terminal  …  (기능 열여섯)
...
| 계산기 | `ExpressionEvaluator`, `CalculatorNumberFormat`, `PercentCalculator`, `BmiCalculator`, `CalculatorSettingsRepository`, 유스케이스 10개 | `DefaultCalculatorSettingsRepository`(`SettingsStore`) | `CalculatorCard`, `CalculatorScreen` |
...
| 세계 시계 | `CivilDate`, `DateCalculator`, `ZoneResolver`, `WorldCities`, 리포지토리 3개, 유스케이스 6개 | `PlatformTimeZoneRepository`(`zoneOffsetSeconds`·`systemZoneId`), `SystemClockRepository`, `DefaultSavedCitiesRepository`(`SettingsStore`) | `WorldClockCard`, `WorldClockScreen` |
